...
export class Icon extends Component {
  ...
  render () {
    const {
      component, title, fill, opacity, shade, state, size, viewBox,
      children, ...rest
    } = this.props
    let props = title != null
    ...
    props.opacity = opacity != null ? opacity
      : shade === 'on-light' ? state === 'focused' ? '.87' : state === 'active'
        ? '.54'
        : state === 'inactive' ? '.38' : state === 'error' ? '1' : undefined
        : shade === 'on-dark' ? state === 'focused' ? '1' : state === 'active'
          ? '.7'
          : state === 'inactive' ? '.5' : state === 'error' ? '1' : undefined
          : undefined
